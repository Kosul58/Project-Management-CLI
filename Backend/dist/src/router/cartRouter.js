import { addProduct, removeProducts, removeProduct, viewCartProduct, calcTotal, viewCartProducts, updateProduct, viewCart, } from "../controllers/cart.js";
...
const cartRouter = async (Command_Prompt) => {
    ...
    switch (Command_Prompt[1]) {
        ...
        case "view":
            let viewResult;
            if (!productid && !userid)
                viewResult = await viewCart();
            if (userid && productid)
                viewResult = await viewCartProduct(productid, userid);
            if (userid && !productid)
                viewResult = await viewCartProducts(userid);
            console.log(viewResult);
            break;
        case "total":
            const totalResult = await calcTotal(userid);
            console.log(totalResult);
            console.log(`Total price of all products in the cart of a user ${userid} is $${totalResult}`);
            break;
        case "update":
