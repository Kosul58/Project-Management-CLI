...
export const createCategory = async (category: CategoryOption) => {
  try {
    ...
    const result = await categoryServices.createCategory(category, "cli");
...
export const readCategories = async () => {
  try {
    const result = await categoryServices.readCategories("cli");
    if (result.length === 0) {
      return {
...
export const readCategory = async (categoryid: string) => {
  try {
    ...
    const result = await categoryServices.readCategory(categoryid, "cli");
...
export const deleteCategory = async (categoryid: string) => {
  try {
    ...
    const result = await categoryServices.deleteCategory(categoryid, "cli");
    if (!result || result.length === 0) {
      return {
        message: "Category update unsuccessfull",
        response: [],
