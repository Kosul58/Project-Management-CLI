...
export const viewOrders = async (userid: string) => {
  try {
    const result = await orderServices.getOrder(userid, "cli");
    if (!result) {
...
export const createOrder = async (userid: string, productid: string) => {
  try {
    ...
    const result = await orderServices.addOrder(userid, productid, "cli");
...
export const createOrders = async (userid: string, products: string[]) => {
  try {
    ...
    const result = await orderServices.addOrders(userid, products, "cli");
...
export const cancelWholeOrder = async (orderid: string, userid: string) => {
  try {
    ...
    const result = await orderServices.removeOrders(orderid, userid, "cli");
    // console.log(result);
    if (!result || result.length === 0) {
      return {
